...
CHECK = {"wide": colors.green | "✔ ", "narrow": colors.green | "✔", "ascii": "[X]"}
UNCHECK = {"wide": colors.red | "✖ ", "narrow": colors.red | "✖", "ascii": "[ ]"}
...
class ConfiguredApplication(cli.Application):
    ...
    config_filename = cli.SwitchAttr(
        ...
        argname="CONFIG",
        help=_("Use file CONFIG for config"),
    )  # noqa: Q000
    verbose = cli.Flag(
        ["-v", "--verbose"],
...
class Content(Mapping):
    ...
    def _get(self):
        ...
        if not os.path.exists(CONTENT_JSON) or self._rebuild_cache:
            ...
            Content._cache = content_endpoint(
                **next(
                    (
                        {"language": lang}
                        for lang in chain(
...
class Status(ApplicationWithApi):
    ...
    def main(self):
        ...
        user["pet"] = (
            user["items"]["currentPet"] if "currentPet" in user["items"] else None
        )
        user["pet"] = content["petInfo"][user["pet"]]["text"] if user["pet"] else ""
        user["pet"] = (
            _("Pet: ") + user["pet"] if user["pet"] else _("No pet")
        )  # noqa: Q000
        user["mount"] = user["items"].get("currentMount", None)
        user["mount"] = (
            content["mountInfo"][user["mount"]]["text"] if user["mount"] else ""
        )
        if user["mount"]:
...
class Dailys(TasksPrint):
    ...
    def domain_format(self, daily):
        ...
        checklist = (
            " {}/{}".format(checklist_done, len(daily["checklist"]))
            if daily["checklist"]
            else ""
        )
        alias = daily.get("alias", "")  # getting the alias
        alias = " (Alias: {})".format(alias) if alias else ""  # format it
        res = _("{0}{1}{text}{2}{3}").format(
            check, score, checklist, alias, **daily
        )  # noqa: Q000
        if not daily["isDue"]:
...
class ToDos(TasksPrint):
    ...
    def domain_format(self, todo):
        ...
        checklist = (
            " {}/{}".format(checklist_done, len(todo["checklist"]))
            if todo["checklist"]
            else ""
        )
        alias = todo.get("alias", "")  # getting the alias
        alias = " (Alias: {})".format(alias) if alias else ""  # format it
        res = _("{1}{0}{text}{2}{3}").format(
            check, score, checklist, alias, **todo
        )  # noqa: Q000
        return res
...
class TasksChange(ApplicationWithApi):
    ...
    noop = cli.Flag(
        ["--dry-run", "--noop"],
        help=_(
            "If passed, won't actually change anything on habitipy server"
        ),  # noqa: Q000
        default=False,
    )
    more_tasks = []  # type: List[Dict[str, Any]]
...
class HabitsAdd(ApplicationWithApi):
    ...
    priority = cli.SwitchAttr(
        ...
        cli.Set("0.1", "1", "1.5", "2"),
        default="1",
        help=_("Priority (complexity) of a habit"),
    )  # noqa: Q000
    direction = cli.SwitchAttr(
        ["-d", "--direction"],
        cli.Set("positive", "negative", "both"),
        default="both",
        help=_("positive/negative/both"),
    )  # noqa: Q000
    ...
    def main(self, *habit: str):
        ...
        res = _(
            "Added habit '{}' with priority {} and direction {}"
        ).format(  # noqa: Q000
...
class HabitsDelete(HabitsChange):
    ...
    def log_op(self, tid):
        return _("Deleted habit {text}").format(
            **self.changing_tasks[tid]
        )  # noqa: Q000
...
class HabitsUp(HabitsChange):
    ...
    def log_op(self, tid):
        return _("Incremented habit {text}").format(
            **self.changing_tasks[tid]
        )  # noqa: Q000
...
class HabitsDown(HabitsChange):
    ...
    def log_op(self, tid):
        ...
        return _("Decremented habit {text}").format(
            **self.changing_tasks[tid]
        )  # noqa: Q000
...
@Dailys.subcommand("add")
class DailysAdd(ApplicationWithApi):
    DESCRIPTION = _("Add a daily <daily>")  # noqa: Q000
    direction = cli.SwitchAttr(
        ["-d", "--direction"],
        cli.Set("positive", "negative", "both"),
        default="both",
    ...
    alias = cli.SwitchAttr(
        ["-a", "--alias"], str, default="", help=_("Alias for the todo")
    )  # noqa: Q000

    def main(self, *habit: str):
        daily_str = " ".join(habit)
        if not daily_str:
            self.log.error(_("Empty habit text!"))  # noqa: Q000
            return 1
        super().main()
        ...
        res = _("Added daily with direction {}").format(  # noqa: Q000
            daily_str, self.direction
        )
        print(prettify(res))
        Habits.invoke(config_filename=self.config_filename)
        return None
...
class DailysUp(DailysChange):
    ...
    def log_op(self, tid):
        return _("Completed daily {text}").format(
            **self.changing_tasks[tid]
        )  # noqa: Q000
...
class DailyDown(DailysChange):
    ...
    def log_op(self, tid):
        return _("Unchecked daily {text}").format(
            **self.changing_tasks[tid]
        )  # noqa: Q000
...
class TodosUp(TodosChange):
    ...
    def log_op(self, tid):
        return _("Completed todo {text}").format(
            **self.changing_tasks[tid]
        )  # noqa: Q000
...
class TodosAdd(ApplicationWithApi):
    ...
    priority = cli.SwitchAttr(
        ...
        cli.Set("0.1", "1", "1.5", "2"),
        default="1",
        help=_("Priority (complexity) of a todo"),
    )  # noqa: Q000
    ...
    def main(self, *todo: str):
        ...
        super().main()
        self.api.tasks.user.post(
            type="todo", text=todo_str, priority=self.priority, alias=self.alias
        )
        res = _("Added todo '{}' with priority {} and alias '{}'").format(
            todo_str, self.priority, self.alias
        )  # noqa: Q000
        print(prettify(res))
        ToDos.invoke(config_filename=self.config_filename)
        return 0
...
class RewardsBuy(TasksChange):
    ...
    def log_op(self, tid):
        return _("Bought reward {text}").format(
            **self.changing_tasks[tid]
        )  # noqa: Q000
...
class RewardsAdd(ApplicationWithApi):
    ...
    cost = cli.SwitchAttr(
        ["--cost"], default="10", help=_("Cost of a reward (gp)")
    )  # noqa: Q000
    ...
    def main(self, *reward: str):
        ...
        res = _("Added reward '{}' with cost {}").format(
            todo_str, self.cost
        )  # noqa: Q000
        print(prettify(res))
        Rewards.invoke(config_filename=self.config_filename)
    ...
    def main(self):
        ...
        try:
            ...
            if isinstance(ret, dict) and ret["status"] == "up":
                print(
                    _("Habitica server {} online").format(self.config["url"])
                )  # noqa: Q000
                return 0
        except (KeyError, requests.exceptions.ConnectionError):
            pass
        msg = _(
            "Habitica server {} offline or there is some issue with it"
        )  # noqa: Q000
...
class Spells(ApplicationWithApi):
    ...
    def main(self):
        ...
        if user_level < 10:
            print(
                _("Your level is too low. Come back on level 10 or higher")
            )  # noqa: Q000
        user_class = user["stats"]["class"]
        user_spells = [
            v for k, v in content["spells"][user_class].items() if user_level > v["lvl"]
        ]
        print(
            _("You are a {} of level {}").format(_(user_class), user_level)
        )  # noqa: Q000
        for spell in sorted(user_spells, key=lambda x: x["lvl"]):
            msg = _("[{key}] {text} ({mana}:droplet:) - {notes}").format(
                **spell
            )  # noqa: Q000
            print(msg)
